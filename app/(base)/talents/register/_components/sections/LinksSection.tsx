    ...
    <section className="section section-links flex flex-col gap-6">
      ...
      <div className="links-list flex flex-col gap-4">
        {fields.map((field, index) => (
          <div key={field.id} className="link-item flex items-start gap-4">
            ...
            <div className="flex-1 bg-bg-primary border border-border-quaternary rounded-lg p-4 flex items-center gap-4">
              ...
              <button
                ...
                onClick={() => handleOpenLink(field.url)}
                title="링크 열기"
              >
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                  <path
              ...
              {fields.length > 1 && (
                <button
                  type="button"
                  className="p-3 rounded-lg hover:bg-bg-tertiary transition-colors shrink-0"
