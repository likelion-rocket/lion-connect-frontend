 ...
 * @param onArrowClick - 화살표 버튼 클릭 핸들러
 * @param index - 카드의 인덱스 (무한 슬라이드에서 투명도 계산용)
 * @param totalCards - 전체 카드 개수
 */
function StartupCard({ startup, onArrowClick, index = 0, totalCards = 1 }: StartupCardProps) {
  ...
  return (
    <div
      ref={cardRef}
      style={{ opacity }}
      className="flex h-[328px] w-[270px] shrink-0 flex-col gap-[10px] rounded-lg border border-border-quaternary bg-bg-secondary p-5 transition-opacity duration-100"
    >
      {/* 상단 영역: 로고 + 화살표 버튼 */}
      <div className="relative flex w-full items-start gap-[69px]">
        ...
        <div className="absolute right-0 top-0 flex items-center gap-[10px]">
          <button
            onClick={() => onArrowClick?.(startup.id)}
            className="flex h-12 w-12 shrink-0 items-center justify-center transition-opacity hover:opacity-70"
            aria-label={`${startup.name} 상세보기`}
          >
