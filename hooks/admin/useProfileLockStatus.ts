...
export function useProfileLockStatus(profileId: number) {
  ...
  return useMutation({
    ...
    onMutate: async (shouldLock: boolean) => {
      console.log(`[useProfileLockStatus] onMutate - profileId: ${profileId}, shouldLock: ${shouldLock}`);
      ...
      queryClient.setQueriesData<AdminUsersResponse>(
        ...
        (old) => {
          ...
          console.log(`[useProfileLockStatus] Updated cache for user ${profileId}:`, updated.content.find(u => u.id === profileId));
          return updated;
        }
      );
    ...
    // 성공 시 로그만 (invalidateQueries 제거)
    onSuccess: (data) => {
      console.log(`[useProfileLockStatus] onSuccess - 서버 응답:`, data);
